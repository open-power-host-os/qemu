                                                  ...
                                                  target_ulong *args)
{
    ...
    if (guest_radix) {
        if (kvm_enabled() && !kvmppc_has_cap_mmu_radix()) {
            error_report("qemu: Guest requested unavailable MMU mode (radix).");
            exit(EXIT_FAILURE);
        }
        spapr_ovec_set(spapr->ov5_cas, OV5_MMU_RADIX_300);
    } else {
        if (kvm_enabled() && kvmppc_has_cap_mmu_radix() && !kvmppc_has_cap_mmu_hash_v3()) {
            error_report("qemu: Guest requested unavailable MMU mode (hash).");
            exit(EXIT_FAILURE);
        }
    }
